...
export function SystemNotificationsCenter({ mode = "overview" }: { mode?: NotificationsPageMode }) {
  ...
  const [readFilter, setReadFilter] = React.useState<ReadFilter>(mode === "unread" ? "unread" : "all");
  ...
  const pageTitle = mode === "list" ? t.openListTitle : mode === "unread" ? t.unreadTitle : t.title;
  const pageSubtitle = mode === "list" ? t.openListDesc : mode === "unread" ? t.unreadDesc : t.subtitle;
  React.useEffect(() => {
    const applyLocale = () => {
  ...
  const resetFilters = React.useCallback(() => {
    setSearch("");
    setReadFilter(mode === "unread" ? "unread" : "all");
    setChannel("all");
    setPriority("all");
    setSort("newest");
  }, [mode]);
  ...
  const hasFilters =
    ...
    priority !== "all" ||
    sort !== "newest";
  const previewRows = mode === "overview" ? filteredNotifications.slice(0, 10) : filteredNotifications;
