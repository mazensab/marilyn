...
/* ============================================================
   📂 marilyn_frontend/app/system/settings/_components/system-settings-client.tsx
   ⚙️ Marilyn Clinics — System Settings Center
   ------------------------------------------------------------
   ✅ Approved Marilyn system workspace pattern aligned with HR Center
   ...
   ✅ Excel .xls export
   ✅ Browser print + Save as PDF through the print dialog
   ✅ Skeleton loading
...
const translations = {
  ar: {
    ...
    subtitle:
      "إدارة الإعدادات العامة والأمان والإشعارات والمستندات والتوطين من واجهات إعدادات النظام الفعلية.",
    badge: "الإدارة المركزية",
    connected: "متصل بواجهة إعدادات النظام",
    refresh: "تحديث",
    ...
    reportTitle: "تقرير إعدادات نظام Marilyn Clinics",
    ...
    confirmResetTitle: "تأكيد إعادة ضبط الإعداد",
    confirmResetAction: "إرجاع الافتراضي",
    cancelReset: "إلغاء",
  },
  en: {
    title: "System settings",
    subtitle:
      "Manage general, security, notification, document, and localization settings through the live system settings APIs.",
    badge: "Central administration",
    connected: "Connected to the system settings API",
    refresh: "Refresh",
    ...
    reportTitle: "Marilyn Clinics System Settings Report",
    ...
    confirmResetTitle: "Confirm setting reset",
    confirmResetAction: "Reset default",
    cancelReset: "Cancel",
  },
} as const;
...
export default function SystemSettingsClient() {
  ...
  const [pendingReset, setPendingReset] =
    React.useState<SettingRecord | null>(null);
  ...
  async function resetSetting(setting: SettingRecord) {
    try {
      ...
      await requestJson<unknown>(
        makeApiUrl(
          `${SETTINGS_ENDPOINT}${setting.id}/reset/`,
        ),
        {
          method: "POST",
          body: {},
  ...
  function exportExcel() {
    ...
    link.href = url;
    link.download = `marilyn-system-settings-${new Date().toISOString().slice(0, 10)}.xls`;
        ...
        <Card className="overflow-hidden rounded-lg border bg-card shadow-none">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="min-w-0">
                <CardTitle className="flex items-center gap-2 text-base font-bold tracking-tight">
                  <Settings className="h-4 w-4 text-[#a57b3d]" />
                  {t.tableTitle}
                </CardTitle>
              ...
              <div className="flex shrink-0 flex-wrap items-center gap-2">
                <Badge variant="outline" className="h-8 rounded-full px-3">
                  ...
                  {t.showing} {formatInteger(previewRows.length)} {t.of}{" "}
                  {formatInteger(apiTotal || settings.length)} {t.rows}
                </Badge>
          ...
          <CardContent className="space-y-4 px-5 pb-5 sm:px-6">
            ...
            <div className="overflow-hidden rounded-lg border bg-background">
              <div className="w-full overflow-x-auto">
                ...
                >
                  <TableHeader>
                    <TableRow>
                      <TableHead className={cn("w-[150px]", alignClass)}>
                        {t.group}
                      </TableHead>
                      <TableHead className={cn("w-[190px]", alignClass)}>
                        {t.key}
                      </TableHead>
                      <TableHead className={cn("w-[250px]", alignClass)}>
                        {t.setting}
                      </TableHead>
                      <TableHead className={cn("w-[120px]", alignClass)}>
                        {t.valueType}
                      </TableHead>
                      <TableHead className={cn("w-[180px]", alignClass)}>
                        {t.value}
                      </TableHead>
                      <TableHead className={cn("w-[180px]", alignClass)}>
                        {t.defaultValue}
                      ...
                      <TableHead className={cn("w-[110px]", alignClass)}>
                        {t.status}
                      </TableHead>
                      <TableHead className={cn("w-[115px]", alignClass)}>
                        {t.updatedAt}
                      </TableHead>
                      <TableHead
                        className={cn(
                          actionStickyClass,
                          "z-10 w-[125px] bg-muted/40 text-center",
                        )}
                      >
                        {t.actions}
                      </TableHead>
                      ...
                      previewRows.map((setting) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <TableCell className={alignClass}>
                              <span className="block truncate rounded-md bg-muted/50 px-2 py-1 text-xs tabular-nums">
                                {displayValue(setting.value, locale)}
                              </span>
                            </TableCell>

                            <TableCell className={alignClass}>
                              <span className="block truncate rounded-md bg-muted/30 px-2 py-1 text-xs tabular-nums text-muted-foreground">
                                {displayValue(
                            ...
                            <TableCell
                              className={cn(
                                actionStickyClass,
                                "z-10 bg-background text-center",
                              )}
                            >
                              <Button
                                variant="outline"
                                size="sm"
                                className={registerOutlineButtonClass}
                                onClick={() => setPendingReset(setting)}
            ...
            <div className="flex items-center justify-between gap-3 text-xs text-muted-foreground">
              <span>
                {t.showing}{" "}
                <strong>{formatInteger(previewRows.length)}</strong>{" "}
                {t.of}{" "}
                <strong>
            ...
            <AlertDialogDescription>
              ...
              {pendingReset ? (
                <span className="mt-2 block font-medium text-foreground">
                  {locale === "ar"
                    ? pendingReset.label_ar || pendingReset.label_en
                    : pendingReset.label_en || pendingReset.label_ar}
                  {" — "}
                  <span dir="ltr">{pendingReset.key}</span>
                </span>
              ) : null}
            </AlertDialogDescription>
          </AlertDialogHeader>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={(event) => {
                ...
                void resetSetting(pendingReset);
              }}
            >
              {resettingId ? (
                <Loader2 className="h-4 w-4 animate-spin" />
